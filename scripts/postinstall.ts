...
const repoShort = SHORT[pkg.name ?? ''] || pkg.name || 'unknown';
...
const SCRIPTS: Record<string, string> = {
  'generate:manual-deploy': 'node node_modules/@sonarmd/star-config/dist/postinstall.js',
  prepare: 'husky',
};
...
const lintStagedRc = path.join(root, '.lintstagedrc.mjs');
const lintStagedContent = 'export {default} from "@sonarmd/star-config/lint-staged";\n';

if (!fs.existsSync(lintStagedRc)) {
  fs.writeFileSync(lintStagedRc, lintStagedContent);
  console.log('@sonarmd/star-config: .lintstagedrc.mjs created');
}
